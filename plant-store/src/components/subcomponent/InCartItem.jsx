...
function InCartItem({items}) {
  ...
  return (
    <div className='bg-white sm:w-[270px] w-[132px] sm:h-[400px] h-[240px] border shadow-md sm:m-3 m-2 flex-none cursor-pointer snap-center relative hover:shadow-lg duration-200'>
      ...
      <button onClick={() => removeFromCart(items.id)} className='w-[100] h-[80] bg-green-700 text-xs text-white rounded-tr-full py-2 px-4 flex justify-center items-center hover:bg-green-600 sm:rounded-sm shadow absolute sm:bottom-3 sm:right-3 bottom-0 border-none active:opacity-75'>Remove from cart</button>
    </div>
  )
}
